...
// Load secrets/credentials from a local `.env` file (gitignored) into
// process.env before any of them are read below. Dependency-free parser so we
// don't pull in `dotenv`. Looks next to main.js (dev) and in the packaged
// resources dir. Existing process.env values always win.
function loadDotEnv() {
  const candidates = [
